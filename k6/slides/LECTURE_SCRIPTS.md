...
### Slide 6: 讓測試結果一目瞭然 - 用 group() 把使用者旅程分段
* **視覺焦點**：QuickPizza 旅程三段（瀏覽首頁 → 取得推薦 → 送出評分）、group 骨架程式碼、三段各自的 p95 長條圖。
* **心智模型**：使用者做的是「一趟旅程」不是「一個請求」；分段之後才知道慢在哪一段。
* **🎤 口播逐字稿**：
  > 「把旅程的每一段用 `group()` 包起來、取個有意義的名字，k6 就會分段統計。每段的停頓不同，流量節奏才像真人。跑的時候加 `--summary-mode=full`，就能看到『取得推薦的 p95 是 312ms，比首頁慢 50%』這種能直接回報的結論。  
  > 提醒：比快慢看 `http_req_duration`，不要看包含停頓的 `group_duration`；group 名稱要固定，不放動態值。」
...
### Slide 8: 看懂儀表板 (8 種經典曲線型態)
* **視覺焦點**：4×2 迷你折線圖：健康線性、飽和平台、崩潰懸崖、尾巴張開、緩慢爬坡、週期鋸齒、錯誤率階梯、吞吐下滑（連線層）。
...
* **🎤 口播逐字稿**：
  ...
  > 其他型態：崩潰懸崖是延遲和錯誤率一起暴衝；尾巴張開是只有 P99 發散；緩慢爬坡是負載不變延遲卻上升，代表資源洩漏；週期鋸齒要去查排程、GC 和快取 TTL；錯誤率階梯代表連線池這類有限資源被用完；RPS 下滑但 P95 持平、blocked 上升，問題在連線建立層，先排除壓測機。」

---

### Slide 9: Soak 判讀 (趨勢比門檻重要)
* **視覺焦點**：P95 緩升但未破門檻的折線圖；三個判讀習慣；「k6 端的果 → 後端的因」對照表。
* **心智模型**：Soak 看斜率不看紅綠燈；負載歸零後不恢復是最硬的證據；k6 只有果，根因要對齊後端數據。
* **🎤 口播逐字稿**：
  > 「全程綠燈也可能是失敗的 Soak：P95 從 300 爬到 700，門檻 800 沒破，但第八小時就會紅。  
  > 三個習慣：量化斜率、一定看恢復、守住窗口。然後拿右邊的表，把 k6 端的訊號對到後端的記憶體、連線池與連線數。」

---

### Slide 10: 儀表板判讀 4 步驟 SOP
...
### Slide 11: Grafana 全視角對齊 (CPU CFS Throttling 破除孤島)
...
### Slide 12: 企業導入實作 Checklist (Takeaway)
...
### Slide 13: 推薦延伸閱讀 (Author's Deep-Dive Articles)
