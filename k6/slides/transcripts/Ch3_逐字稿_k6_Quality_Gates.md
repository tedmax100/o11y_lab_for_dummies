...
* **畫面焦點**：左側 100 格方塊（99 格灰色＝10ms、1 格紅色＝10,000ms）；右側兩張數字卡：平均值 109.9ms「看起來很快」vs 最慢的那 1 個 10,000ms；下方四個重點：平均值掩蓋極端值、p(95) 發版門檻、p(99) 尾端防線、微服務放大效應。
...
> 因此，從今天開始，我們設定所有效能門檻，一律以 P95 或 P99 為依據！」
...
* **畫面焦點**：2×2 四張卡片：Counter（累積計數）、Gauge（即時數值）、Rate（成功／失敗比率）、Trend（耗時分佈），每張附一行 `.add()` 程式碼。
* **螢幕動作**：【動作：依序介紹四種指標類別】。
...
* **螢幕動作**：【動作：圈選大括號內的 Tag 過濾條件】→【動作：指向下方的 Group 過濾】。
...
> 
> 再看下方，還記得第一章用 `group()` 把使用者旅程切成好幾段嗎？group 裡面送出的每個請求，k6 都會自動打上 `group` 標籤，所以也能直接拿來設門檻：  
> `'http_req_duration{group:::01_核心結帳交易}': ['p(95)<1000']`。  
> 注意這裡是三個冒號——標籤名 `group` 加一個冒號，k6 又會在 group 名稱前面自動加上兩個冒號。  
> 
> 那 tag 跟 group 怎麼選？一句話：**group 回答『旅程中的哪一段慢』，tag 回答『哪一類 API 慢』。** 同一個結帳 API 可能在好幾段旅程裡都會被呼叫，想不分段落統一管它，就用 tag；想看某一段旅程整體表現，就用 group。  
> 
> 最後提醒一個陷阱：group 名稱打錯，k6 不會報錯，門檻抓不到任何請求，摘要會顯示 `p(95)=0` 然後打勾通過。改了 group 名稱，記得回頭確認門檻的數字不是 0。  
> 
...
> 當特定指標（例如投影片上的業務成功率掉到 95% 以下）觸發門檻時，k6 會毫不猶豫**立即腰斬整個測試**，不再進行後續無效的空跑！同時，我們通常會搭配 `delayAbortEval: '5s'`，給系統 5 秒鐘的預熱緩衝，避免因為剛啟動時的冷啟動抖動造成誤殺。這才是一套成熟、具備成本防禦意識的架構設計！」
...
> 任務一：用 `FAIL_SLO=false` 執行 `k6/demos/ch3_quality_gates_exit99.js`，觀察 Tags 與 Group 各自的 p95、四大自訂指標，並用 5 步驟判讀這份摘要，結束碼是 0；  
> 任務二：改用 `FAIL_SLO=true`，確認只有 critical 端點打紅叉，並用 `echo $?` 親眼看到 Exit Code 99；  
> 任務三：改用 `ABORT_TEST=true`，體驗 `abortOnFail` 在幾秒內就把測試腰斬的即時熔斷。  
> 
